...
var psmodels = require('./modelData/photoApp.js').psmodels;
...
app.get('/user/list', function (request, response) {
  response.status(200).send(psmodels.userListModel());
  return;
});
...
app.get('/user/:id', function (request, response) {
  ...
  var user = psmodels.userModel(id);
  if (user === null) {
...
app.get('/photosOfUser/:id', function (request, response) {
  var id = request.params.id;
  var photos = psmodels.photoOfUserModel(id);
  if (photos.length === 0) {
